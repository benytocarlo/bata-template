                ...
                <div class="promoborder">
                    <div class="indent">
                        <a href="{$minicSlider.promo_products.$k.link}" class="imgLink">
                            <img src="{$link->getImageLink($minicSlider.promo_products.$k.link_rewrite, $minicSlider.promo_products.$k.id_image)}" alt="{$minicSlider.promo_products.$k.legend|escape:html:'UTF-8'}" title="{$minicSlider.promo_products.$k.name|escape:html:'UTF-8'}" />
                        </a>
                        <div class="productInfo smooth02 {if !isset($minicSlider.promo_products.$k.specific_prices.price)}not_special{/if}">
                            ...
                            <div class="info smooth02">
                                <div class="clearfix">
                                    <span class="manufacturer_name trajan ellipsis">{$minicSlider.promo_products.$k.name|escape:html:'UTF-8'|truncate:30:'...':true}
                                    </span>
                                    ...
                                    {/if}
                                    <span class="name ellipsis">{if isset($minicSlider.promo_products.$k.manufacturer_name)}{$minicSlider.promo_products.$k.manufacturer_name|truncate:30}{/if}</span>
                                    {if !$PS_CATALOG_MODE}
                                    <a class="button btn-red lmromandemi" href="{$minicSlider.promo_products.$k.link}">{l s='CÓMPRALOS' mod='pk_promominicslider'}</a>
                                    {else}
                                    <a class="button btn-red lmromandemi" href="{$minicSlider.promo_products.$k.link}">{l s='VER' mod='pk_promominicslider'}</a>
                                    {/if}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="price_with_reduction smooth02 trajan">
                            <div class="hwhite">
                            <span class="now trajan">{$minicSlider.promo_products.$k.name|escape:html:'UTF-8'|truncate:30:'...':true}</span>
                            <span class="description">{if isset($minicSlider.promo_products.$k.manufacturer_name)}{$minicSlider.promo_products.$k.manufacturer_name|truncate:30}{/if}</span>
                            <span class="price lmromandunh">
                                {displayPrice price=$price}                             
                            </span>
                            {if !$PS_CATALOG_MODE}
                            <a class="button lmromandemi btnShopping" href="{$minicSlider.promo_products.$k.link}">{l s='CÓMPRALOS' mod='pk_promominicslider'}</a>
                            {else}
                            <a class="button lmromandemi btnShopping" href="{$minicSlider.promo_products.$k.link}">{l s='VER' mod='pk_promominicslider'}</a>
                            {/if}
                            </div>
                        </div>
                        {/if}
