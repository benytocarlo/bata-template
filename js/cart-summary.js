...
$(document).ready(function(){
    ...
    $('.container-spinner .custom-plus').on('click', function() {
        var qty = $(this).prev().val();
        if (!isNaN(qty)) {
            qty = parseInt(qty) + 1;
            $(this).prev().val(qty);            
            updateQty(qty, true, $(this).prev().get(0));
        }
    });
    
    $('.container-spinner .custom-less').on('click', function() {
        var qty = $(this).next().val();        
        if (!isNaN(qty)) {
            if (parseInt(qty) > 1) {
                qty = parseInt(qty) - 1;
                $(this).next().val(qty);                
                updateQty(qty, true, $(this).next().get(0));
            }
        }
    });
