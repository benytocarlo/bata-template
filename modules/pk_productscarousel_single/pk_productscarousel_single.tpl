...
<div class="content-csimple">
<div class="wrapcenter-bata">
  <div class="photogirl" data-stellar-ratio="1.5"></div>
  <div class="nameprogirl">
   ...
   <a href="/3-mujer">{l s='VER TODOS LOS PRODUCTOS' mod='pk_productscarousel_single'}</a>
  </div>
  <div id="productsCarousel_single" class="carouselDesktop load-animate homemodule">
      <!--table class="title-table">
        <tr>
          ...
          {l s='Productos destacados' mod='pk_productscarousel_single'}
        {elseif $pr_type == "new"}
          {l s='New Products' mod='pk_productscarousel_single'}
        {elseif $pr_type == "spe"}
          {l s='On Sale' mod='pk_productscarousel_single'}
        {elseif $pr_type == "bes"}
          {l s='Bestsellers' mod='pk_productscarousel_single'}
        {/if}</h3></td>
          <td class="w50p"><span class="left-wing title-wing"></span></td>
        </tr>
      </table-->
      <div class="wht-bg slidetop">
        <div class="indent">
                    ...
                    <div class="carouselContainer" onclick="window.location='{$productLink}'">
                      <div class="slide-animate">
                        ...
                        <a href="{$productLink}" title="{$set.data.legend}" class="product_image p{$set.data.id_product}">
                          ...
                          {if ($countdown == true)}
                              ...
                              {if isset($set.data.specific_prices.to) && ($to[0] != "0000")}
                                ...
                                $(document).ready(function(){
                                  $(function() {
                                    $('#productsCarousel_single .countdown-{$set.data.id_product}').countdown({
                          ...
                          <span class="labels">
                            {*if isset($set.data.new) && $set.data.new == 1}<span class="pk-new sec_bg">{l s='New' mod='pk_productscarousel_single'}</span>{/if*}
                          ...
                          <div class="function_buttons smooth02 altview">
                              ...
                              <div class="function_button dib product_wishlist sec_bshadow_hvr smooth02 sec_bg_hvr{if $isInWishList[$set.data.id_product] == 1} sec_bg sec_bshadow remove{/if}">
                                    <a href="#" class="wishlist_button addToWishlist" title="{if $isInWishList[$set.data.id_product] == 1}{l s='Este producto ya se encuentra en tu lista de deseos' mod='pk_productscarousel_single'}{else}{l s='Agregar a la lista de deseos' mod='pk_productscarousel_single'}{/if}">
                              ...
                              {*if ($favorite.install == "installed") && ($favorite.enable == "enabled")}
                                <div class="function_button dib product_like sec_bshadow_hvr smooth02 sec_bg_hvr {if $isFav[$set.data.id_product] == 1} sec_bg sec_bshadow{/if}">
                                  <a title="{l s='Add this product to my favorites' mod='pk_productscarousel_single'}" href="#" class="addfav dib{if $isFav[$set.data.id_product] == 1} hidden{/if}">
                                    <svg class="svgic svgic-wishlist"><use xlink:href="#si-wishlist"></use></svg>
                                  </a>
                                  <a title="{l s='Remove product from my favorites' mod='pk_productscarousel_single'}" href="#" class="remfav  dib{if $isFav[$set.data.id_product] != 1} hidden{/if}">
                                    <svg class="svgic svgic-wishlist"><use xlink:href="#si-wishlist"></use></svg>
                                  </a>
                                  </div>
                              {/if*}
                      ...
                      {*
                        {if ($comments["install"] == "installed") && ($comments["enable"] == "enabled") && ($p_rating == 1)}
                          <!--span cla/15-temporada-nueva-coleccionss="rate_block smooth05">
                        ...
                        <a class="f_title ellipsis" href="{$productLink}" title="{$set.data.legend}">{$set.data.name|escape:htmlall:'UTF-8'|truncate:35}</a>
                          ...
                          <div class="carousel-buttons smooth02">
                            {if ($set.data.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $set.data.available_for_order AND !isset($restricted_country_mode) AND $set.data.minimal_quantity == 1 AND $set.data.customizable != 2 AND !$PS_CATALOG_MODE}
                              {if ($set.data.quantity > 0 OR $set.data.allow_oosp)}
                                <a class="exclusive ajax_add_to_cart_button button dib" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$set.data.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" data-id-product="{$set.data.id_product|intval}"><span>{l s='Add to cart' mod='pk_productscarousel_single'}</span></a>{else}<a href="{$set.data.link}" class="button dib exclusive">{l s='View' mod='pk_productscarousel_single'}</a>{/if}{else}<a href="{$set.data.link}" class="button exclusive dib">{l s='Details' mod='pk_productscarousel_single'}</a>{/if}{if ($favorite.install == "installed") && ($favorite.enable == "enabled")}<div class="function_button normview product_like{if $isFav[$set.data.id_product] == 1} active{/if}"><a title="{l s='Add this product to my favorites' mod='pk_productscarousel_single'}" href="#" class="addfav sec_bg_hvr dib button{if $isFav[$set.data.id_product] == 1} hidden{/if}"><svg class="svgic svgic-like"><use xlink:href="#si-like"></use></svg></a><a title="{l s='Remove product from my favorites' mod='pk_productscarousel_single'}"  href="#" class="remfav sec_bg_hvr dib button{if $isFav[$set.data.id_product] != 1} hidden{/if}"><svg class="svgic svgic-like"><use xlink:href="#si-like"></use></svg></a></div>{/if}{if ($wishlist.install == "installed") && ($wishlist.enable == "enabled")}<div class="function_button normview product_wishlist{if $isInWishList[$set.data.id_product] == 1} active{/if}"><a href="#" class="wishlist_button sec_bg_hvr dib button addToWishlist" onclick="WishlistCart('wishlist_block_list', 'add', '{$set.data.id_product|intval}', false, 1, '.p{$set.data.id_product|intval} .image-cover'); return false;" data-wishid="{$set.data.id_product|intval}" title="{if $isInWishList[$set.data.id_product] == 1}{l s='This product is already in your wishlist' mod='pk_productscarousel_single'}{else}{l s='Add to Wishlist' mod='pk_productscarousel_single'}{/if}"><svg class="svgic svgic-wishlist"><use xlink:href="#si-wishlist"></use></svg></a>
                            </div>
                            {/if}
                          </div>
                          {/if}
                          ...
                          <!-- END BESTSELLERS -->
                        {else}
                          <a class="f_title ellipsis normview" href="{$productLink}" title="{$set.data.legend}">{$set.data.name|escape:htmlall:'UTF-8'|truncate:35}</a>
                          {if $set.data.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
                            {if isset($p_price) && ($p_price == 1)}
                              <div class="content_price smooth02 price-bg{if isset($set.data.specific_prices.price)} oldprice-exist{/if}">
                                {if isset($set.data.specific_prices.price)}
                            ...
                            {/if}
                          {/if}
                          <a class="f_title ellipsis altview" href="{$productLink}" title="{$set.data.legend}">{$set.data.name|escape:htmlall:'UTF-8'|truncate:35}</a>
                          {if isset($p_button) && ($p_button == 1)}
                          <div class="carousel-buttons smooth02">
                            {if ($set.data.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $set.data.available_for_order AND !isset($restricted_country_mode) AND $set.data.minimal_quantity == 1 AND $set.data.customizable != 2 AND !$PS_CATALOG_MODE}
                              {if ($set.data.quantity > 0 OR $set.data.allow_oosp)}
                                <a class="exclusive ajax_add_to_cart_button button dib" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$set.data.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" data-id-product="{$set.data.id_product|intval}"><span>{l s='Add to cart' mod='pk_productscarousel_single'}</span></a>
                              {else}
                                <a href="{$set.data.link}" class="button exclusive">{l s='View' mod='pk_productscarousel_single'}</a>{/if}
                              {/if}
                              {if ($favorite.install == "installed") && ($favorite.enable == "enabled")}<div class="function_button normview product_like{if $isFav[$set.data.id_product] == 1} active{/if}"><a title="{l s='Add this product to my favorites' mod='pk_productscarousel_single'}" href="#!" class="addfav sec_bg_hvr button{if $isFav[$set.data.id_product] == 1} hidden{/if}"><svg class="svgic svgic-wishlist"><use xlink:href="#si-wishlist"></use></svg></a><a title="{l s='Remove product from my favorites' mod='pk_productscarousel_single'}"  href="#!" class="remfav sec_bg_hvr button{if $isFav[$set.data.id_product] != 1} hidden{/if}"><svg class="svgic svgic-wishlist"><use xlink:href="#si-wishlist"></use></svg></a>
                            </div>{/if}
                            {if ($wishlist.install == "installed") && ($wishlist.enable == "enabled")}<div class="function_button normview product_wishlist{if $isInWishList[$set.data.id_product] == 1} active{/if}">
                            <a href="#" class="wishlist_button sec_bg_hvr button addToWishlist dib" onclick="WishlistCart('wishlist_block_list', 'add', '{$set.data.id_product|intval}', false, 1, '.p{$set.data.id_product|intval} .image-cover'); return false;" data-wishid="{$set.data.id_product|intval}" title="{if $isInWishList[$set.data.id_product] == 1}{l s='This product is already in your wishlist' mod='pk_productscarousel_single'}{else}{l s='Add to Wishlist' mod='pk_productscarousel_single'}{/if}"><svg class="svgic svgic-like"><use xlink:href="#si-like"></use></svg></a>
                            </div>
                            {/if}
                          </div>
                          {/if}
                        ...
                        <span class="priceproduct lmroman">{displayPrice price=$set.data.price}</span> 
                          ...
                          <div class="carousel-buttons hvr-shop smooth02">
                          	{if !$PS_CATALOG_MODE}
                          		{if ($set.data.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $set.data.available_for_order AND !isset($restricted_country_mode) AND $set.data.minimal_quantity == 1 AND $set.data.customizable != 2}
                          			<a href="{$set.data.link}" class="btn-buy-single">{l s='COMPRAR' mod='pk_productscarousel_single'}</a>                          			
                                {/if}
                                {if ($set.data.quantity > 0 OR $set.data.allow_oosp)}
                                	<a class="exclusive ajax_add_to_cart_button button dib" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$set.data.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" data-id-product="{$set.data.id_product|intval}">
                                		<span>{l s='AGREGAR A LA BOLSA' mod='pk_productscarousel_single'}</span>
                                	</a>
                                {/if}
                                {if ($wishlist.install == "installed") && ($wishlist.enable == "enabled")}
                                	<div class="function_button normview product_wishlist{if $isInWishList[$set.data.id_product] == 1} active{/if}"><a href="#" class="wishlist_button sec_bg_hvr dib button addToWishlist" onclick="WishlistCart('wishlist_block_list', 'add', '{$set.data.id_product|intval}', false, 1, '.p{$set.data.id_product|intval} .image-cover'); return false;" data-wishid="{$set.data.id_product|intval}" title="{if $isInWishList[$set.data.id_product] == 1}{l s='Este producto ya se encuentra en tu lista de deseos' mod='pk_productscarousel_single'}{else}{l s='Agregar a tu lista de deseos' mod='pk_productscarousel_single'}{/if}"><svg class="svgic svgic-like"><use xlink:href="#si-like"></use></svg></a>
                                	</div>
                              	{/if}
                          	{else}
                          		<a href="{$set.data.link}" class="custom-view-product-list">{l s='VER DETALLE' mod='pk_productscarousel_single'}</a>
                          	{/if}
                          </div>
                          {/if}
                      </div>
          ...
          {else}
            <p class="alert alert-warning">{l s='No hay productos en estos momentos' mod='pk_productscarousel_single'}</p>
          {/if}
        ...
        <div class="promo30g">
            <p>
              {l s='Suscríbete a nuestro ' mod='pk_productscarousel_bottom'}
              <span>{l s='Newsletter ' mod='pk_productscarousel_bottom'}</span>              
              {l s='y entérate primero de nuestras ofertas.' mod='pk_productscarousel_bottom'}
            </p>
          </div>
  </div>
</div><!--End wrapcenter-->
