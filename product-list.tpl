...
{if isset($products)}
	...
	{foreach from=$products item=product name=products}		
		...
		<li class="ajax_block_product{if $smarty.foreach.products.iteration%$nbItemsPerLine == 0} last-in-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLine == 1} first-in-line{/if}{if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModulo)} last-line{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLineTablet == 0} last-item-of-tablet-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLineTablet == 1} first-item-of-tablet-line{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLineMobile == 0} last-item-of-mobile-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLineMobile == 1} first-item-of-mobile-line{/if}{if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModuloMobile)} last-mobile-line{/if}" data-productid="{$product.id_product}" data-rewrite="{$product.link_rewrite}">			
			<div class="fix-bug-hover"><div class="product-container {if $PS_CATALOG_MODE}in-catalog-mode{/if} {if $count%4 == 0} custom-last-item{/if}" itemscope itemtype="http://schema.org/Product">
				...
				<div class="center_block">
					<div class="product-image-container">
						<div style="height: 57px;">
							<span class="container-manufacturer">
                        		<img class="dib" src="{$img_manu_dir}{$product.id_manufacturer}.jpg" alt="{$product.manufacturer_name}" />
							</span>
						</div>
						<a class="product_img_link"	href="{$product.link|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}" itemprop="url">
							<img class="replace-2x img-responsive pimg-{$product.id_product}" src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_default')|escape:'html':'UTF-8'}" alt="{if !empty($product.legend)}{$product.legend|escape:'html':'UTF-8'}{else}{$product.name|escape:'html':'UTF-8'}{/if}" title="{if !empty($product.legend)}{$product.legend|escape:'html':'UTF-8'}{else}{$product.name|escape:'html':'UTF-8'}{/if}" height="115" width="180" itemprop="image" />
								...
								{if (((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}
									{if isset($product.specific_prices) && $product.specific_prices && isset($product.specific_prices.reduction) && $product.specific_prices.reduction}
										{if $product.specific_prices.reduction_type == 'percentage'}
											<span class="pk-reduction">-{$product.specific_prices.reduction * 100}%</span>
										{/if}
									{/if}
								{/if}
							</div>							
						...
						<p class="description-product description-product-grid">{$product.description_short|strip_tags:'UTF-8'|truncate:70}</p>	
					</div>
					<div class="product-info">
						...
						{/if}
						<p class="product-desc description-product-list" itemprop="description">
					...
					<div class="right_block">							
						{if (((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}
							<div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="content_price">
								<div class="separator-product-price">
									{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
										...
										{if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span><br/><meta itemprop="priceCurrency" content="{$priceDisplay}" />
										{if isset($product.specific_prices) && $product.specific_prices && isset($product.specific_prices.reduction) && $product.specific_prices.reduction}
										<span class="precio-oferta lmroman">PRECIO OFERTA</span>
										{/if}
									{/if}	
								</div>
							</div>
						...
						<div class="button-container">							
                            {if !$PS_CATALOG_MODE}
                            <a class="custom-view-product dib button btn-default" href="{$product.link|escape:'html':'UTF-8'}" title="{l s='Comprar'}"><i></i>{l s='Comprar'}</a>
                            {else}
                            <a class="custom-view-product-list" href="{$product.link|escape:'html':'UTF-8'}" title="{l s='Agregar a la bolsa'}" >
                                {l s='Ver Detalle'}
                            </a><br class="lst" />
                            {/if}							
								...
								{if ($product.allow_oosp || $product.quantity > 0)}
									{if isset($static_token)}
										<a class="ajax_add_to_cart_button" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart'}" data-id-product="{$product.id_product|intval}">
											{l s='Agregar a la bolsa'}
										</a><br class="lst" />
									{else}
										<a class="ajax_add_to_cart_button" href="{$link->getPageLink('cart',false, NULL, 'add=1&amp;id_product={$product.id_product|intval}', false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Agregar a la bolsa'}" data-id-product="{$product.id_product|intval}">
											{l s='Agregar a la bolsa'}
										</a><br class="lst" />
									{/if}
								{/if}
