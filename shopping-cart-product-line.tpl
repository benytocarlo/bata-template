...
<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}" class="cart_item{if isset($productLast) && $productLast && (!isset($ignoreProductLast) || !$ignoreProductLast)} last_item{/if}{if isset($productFirst) && $productFirst} first_item{/if}{if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0} alternate_item{/if} address_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if}">
	<td class="cart_product">
		<div class="mobile-title">{l s='Product'}</div>
	...
	<td class="cart_description">
		<div class="mobile-title">{l s='Description'}</div>        
        ...
        {if $product.manufacturer}<p class="roboto"><strong>{$product.manufacturer}</strong></p>{/if}
        {if $product.reference}<p class="cart_ref roboto">{l s='SKU'}: #{$product.reference|escape:'html':'UTF-8'}</p>{/if}        
		{if isset($product.custom_attributes) && $product.custom_attributes}
            {foreach $product.custom_attributes as $attr}
            <p class="roboto">{$attr|escape:'html':'UTF-8'}</p>
            {/foreach}
        {/if}
	</td>
	{if $PS_STOCK_MANAGEMENT}
		<td class="cart_avail hidden"><span class="{if $product.quantity_available <= 0}label label-warning{else}label label-success{/if}">{if $product.quantity_available <= 0}{if $product.allow_oosp}{$product.available_later}{else}{l s='Out of stock'}{/if}{else}{if isset($product.available_now) && $product.available_now}{$product.available_now}{else}{l s='In Stock'}{/if}{/if}</span></td>
	{/if}
	<td class="cart_unit" data-title="{l s='Unit price'}">
		<div class="mobile-title">{l s='Unit price'}</div>
	...
	<td class="cart_quantity text-center">
		<div class="mobile-title">{l s='Quantity'}</div>
		...
		{else}
			...
			{if !isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed > 0}
                ...
                <div class="container-spinner">
                    <a class="custom-less button-cart-qty" href="javascript:;">-</a>
                    <input size="2" type="text" autocomplete="off" class="cart_quantity_input form-control grey" onKeyPress="return onlyNumbers(event)" maxlength="4" value="{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}{$customizedDatas.$productId.$productAttributeId|@count}{else}{$product.cart_quantity-$quantityDisplayed}{/if}"  name="quantity_{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}" />
                    <a class="custom-plus button-cart-qty" href="javascript:;">+</a>
                <div>
	...
	<td class="cart_total" data-title="{l s='Total'}">
		<div class="mobile-title">{l s='Total'}</div>
