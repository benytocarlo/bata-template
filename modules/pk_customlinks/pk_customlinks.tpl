...
<div id="pk_customlinks" class="dib">
	<ul>
		...
		<li class="pk_account dd_el dib smooth02 main_bg_hvr">
			<a href="#" title="{l s='My Account' mod='pk_customlinks'}"><svg class="svgic svgic-account"><use xlink:href="#si-account"></use></svg><span>{l s='My Account' mod='pk_customlinks'}</span></a>
			...
			<a href="{$wishlist_link}" title="{l s='My Wishlist' mod='pk_customlinks'}"><svg class="svgic svgic-wishlist"><use xlink:href="#si-like"></use></svg><span>{l s='My Wishlist' mod='pk_customlinks'} (<span class="wlQty">{if !empty($pk_wishlist["wishlist_products"])}{count($pk_wishlist["wishlist_products"])}{else}0{/if}</span>)</span></a>
		...
		</li>{/if}{if $main_links.mywtl == true}<li class="pk_watchlist dd_el dib smooth02 main_bg_hvr"><a href="#" title="{l s='Recently viewed products' mod='pk_customlinks'}"><svg class="svgic svgic-eye"><use xlink:href="#si-eye"></use></svg><span>{l s='Watch List' mod='pk_customlinks'} <span>({count($watchlist)})</span></span></a>
		...
		</li>{/if}{if $main_links.mycmp == true}<li class="pk_compare dd_el dib smooth02 main_bg_hvr"><a href="{$link->getPageLink('products-comparison')|escape:'html':'UTF-8'}" title="{l s='Compare List' mod='pk_customlinks'}"><svg class="svgic svgic-compare"><use xlink:href="#si-compare"></use></svg><span>{l s='Compare List' mod='pk_customlinks'} (<span class="total-compare-val">{count($compared_products)}</span>)</span></a>
		...
		</li>{/if}{foreach from=$customlinks_links item=blocklink_link}{if isset($blocklink_link.$lang)}<li class="pk_account dd_el dib smooth02 main_bg_hvr custom_link_added">
				<a href="{$blocklink_link.url|escape}"{if $blocklink_link.newWindow} onclick="window.open(this.href);return false;"{/if}><span>{$blocklink_link.$lang|escape}</span></a>
			</li>{/if}{/foreach}
	</ul>
</div>
<!-- /Block customlinks module -->
