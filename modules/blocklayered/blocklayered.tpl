			...
			<div>
				...
				<div id="enabled_filters">
					<div class="border-white">
						<div class="cnt_layered_subtitle">
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{else}
						<div {if $filter.type == 'category'}class="category"{/if}>
						{/if}
						<div class="border-white">
							<div class="cnt_layered_subtitle custom-{$filter.type}">								
								{if $filter.type == 'category'}
								<span class="layered_subtitle_small">{$filter.name|escape:html:'UTF-8'}</span>
								<span class="layered_subtitle">{$xp_category_name|escape:html:'UTF-8'}</span>
								{else}
                                    {if $filter.type == 'manufacturer'}
                                    <span class="layered_subtitle">{l s='MARCA' mod='blocklayered'}</span>
                                    {else}
								...
								{foreach from=$filter.values key=id_value item=value name=fe}
									...
									<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
										...
										<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" data-rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
											{if !$value.nbr && $filter.type != 'category'}
											{$value.name|escape:html:'UTF-8'}{if $layered_show_qties}<span> (<span class="count-product">{$value.nbr}<span>)</span>{/if}
											{else}
											<a href="{$value.link}" data-rel="{$value.rel}">{$value.name|escape:html:'UTF-8'}{if $layered_show_qties && $filter.type != 'category'}<span> (<span class="count-product">{$value.nbr}</span>)</span>{/if}</a>
											{/if}
										</label>
									</li>
									{/if}
									...
									<select class="select" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
										...
										{if $value.nbr || !$hide_0_values}
											<option style="color: {if isset($value.color)}{$value.color}{/if}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}_{$filter.id_key}" {if isset($value.checked) && $value.checked}selected="selected"{/if} {if !$value.nbr}disabled="disabled"{/if}>
												{$value.name|escape:html:'UTF-8'}{if $layered_show_qties && $filter.type != 'category'}<span> (<span class="count-product">{$value.nbr}</span>)</span>{/if}</a>
											</option>
										{/if}
										{/foreach}
									</select>
						...
						{else}
							{if $filter.filter_type == 0}
								...
								<div class="container-custom-slider">
									<span id="layered_{$filter.type}_range_from" class="price_range float-left">{displayPrice price=$filter.min}</span>
									<span id="layered_{$filter.type}_range_to" class="price_range float-right">{displayPrice price=$filter.max}</span>
								</div>
								<script type="text/javascript">								
								{literal}
									...
									addSlider('{/literal}{$filter.type}{literal}',{										
										...
										slide: function( event, ui ) {											
											...
											$('#layered_{/literal}{$filter.type}{literal}_range_from').text(from);
											$('#layered_{/literal}{$filter.type}{literal}_range_to').text(to);
										},
										stop: function () {
											reloadContent();
										}
