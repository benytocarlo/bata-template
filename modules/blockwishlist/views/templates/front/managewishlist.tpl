...
{if $products}
	...
	<div class="wishlistLinkTop">
		...
		<p class="wishlisturl">{l s='Permalink' mod='blockwishlist'}: <input type="text" value="{$base_dir_ssl}module/blockwishlist/view?token={$token_wish|escape:'html':'UTF-8'}" style="width:540px;" readonly="readonly" /><a href="#" id="showSendWishlist" class="button_account exclusive" onclick="WishlistVisibility('wl_send', 'SendWishlist'); return false;" title="{l s='Send this wishlist' mod='blockwishlist'}">{l s='Send this wishlist' mod='blockwishlist'}</a></p>
				...
				<div class="clearfix">
					...
					<div class="product_infos">
						...
						{if isset($product.attributes_small)}
							<a href="{$link->getProductlink($product.id_product, $product.link_rewrite, $product.category_rewrite)|escape:'html'}" title="{l s='Product detail' mod='blockwishlist'}">{$product.attributes_small|escape:'html':'UTF-8'}</a><br />
						{/if}
							{l s='Quantity' mod='blockwishlist'}: <input type="text" id="quantity_{$product.id_product}_{$product.id_product_attribute}" value="{$product.quantity|intval}" size="3"  />
							<br /><br />
