...
{if $errors|@count == 0}
	...
	<div id="primary_block" class="row" itemscope itemtype="http://schema.org/Product">
					...
					<span id="view_full_size">
						{if $jqZoomEnabled && $have_image && !$content_only}
							<a class="jqzoom" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" rel="gal1" href="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_'|cat:$cookie->img_name)|escape:'html':'UTF-8'}" itemprop="url">
								<img itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_'|cat:$cookie->img_name)}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}"/>
							</a>
						{else}
							<img id="bigpic" itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_'|cat:$cookie->img_name)}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" width="{$largeSize.width}" height="{$largeSize.height}" />
							{if !$content_only}
		...
		<!-- left infos -->
		<div id="pb-left-column" class="col-xs-12 col-sm-4 info-ficha">
			<div class="wrapper-name-price">
				<div class="name-product">
					...
					<p id="product_reference"{if isset($groups) || !$product->reference} style="display: none;"{/if}>
						<label>{l s='Código producto'}: </label>
						<span class="editable" itemprop="sku">{$product->reference|escape:'html':'UTF-8'}</span>
					</p>                    
					<p id="availability_statut">
						<span id="availability_label_stock">{l s='Producto:'}</span>
                        <span id="availability_label" class="value_name">{l s='Disponible'}</span>
						<span id="availability_value"{if $product->quantity <= 0} class="warning_inline"{/if}>
						{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='Sin stock'}{/if}{else}{$product->available_now}{/if}
						</span>
						<!-- number of item in stock -->
						{*if ($display_qties == 1 && !$PS_CATALOG_MODE && $product->available_for_order)}
						<span id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}>
							(<span id="quantityAvailable">{$product->quantity|intval}</span>
							<span {if $product->quantity > 1} style="display: none;"{/if} id="quantityAvailableTxt">{l s='Disponible'}</span>
							<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='Disponible'}</span>)
						</span>
						{/if*}
						{if $PS_STOCK_MANAGEMENT}
						{hook h="displayProductDeliveryTime" product=$product}
						<span class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties || $product->quantity <= 0) || $allow_oosp || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Atención: pocos prodcutos en stock!'}</span>
						{/if}
					</p>
				...
				<div class="price-product">
					<div class="price-ficha"><p class="our_price_display" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
					...
					<div class="manufacturer-ficha">
					{if isset($product->manufacturer_name) && $product->manufacturer_name != ""}
					<p class="product-manufacturer">
						 <!--span class="value_name">{l s='Marca:'}</span> <a href="{$link->getmanufacturerLink($product->id_manufacturer)|escape:'htmlall':'UTF-8'}" itemprop="brand" itemscope itemtype="http://schema.org/Organization"><span itemprop="name">{$product->manufacturer_name}</span></a-->
						 <img class="dib marca-ficha" src="{$img_manu_dir}{$product->id_manufacturer}.jpg" alt="" />
					</p></div>
					{/if}
			...
			<div id="pb-right-column1" class="col-xs-12 col-sm-4 col-md-3">
				<div class="box-info-product">
                    {if isset($groups) || !$PS_CATALOG_MODE}
					<div class="product_attributes clearfix">						
						{if isset($groups)}
							<!-- attributes -->
							<div id="attributes">
								<div class="clear"></div>
								{foreach from=$groups key=id_attribute_group item=group name=sizes}
								{assign var="counter" value=$smarty.foreach.sizes.iteration}
									{if $group.attributes|@count}
										<fieldset class="attribute_fieldset">
											...
											{if $counter == 1}
											<a href="#" class="size-verification">{l s="Verifica tu talla"}</a>
											{/if}
										</fieldset>
									{/if}
								{/foreach}
							...
							{/if}
							{if !$PS_CATALOG_MODE && (($product->show_price && !isset($restricted_country_mode)) || isset($groups) || $product->reference || (isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS))}
							<!-- quantity wanted -->
					...
					{/if}
					<!--&& ($accessory.allow_oosp || $accessory.quantity > 0)-->
					{if !$PS_CATALOG_MODE}
					<div class="box-cart-bottom">                        
                         <div class="function_button btn-heart addToWishlist sec_bshadow_hvr smooth02 sec_bg_hvr">
                            <a title="{l s='Agrega este producto a la lista de deseos' mod='pk_productscarousel_bottom'}" href="#" class="addfav dib">
                              <svg class="svgic svgic-like"><use xlink:href="#si-like"></use></svg>
                            </a>
                         </div>
						
						
                         <a href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id|intval}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" class="btn-buy-single">{l s='COMPRAR'}</a>
                         ...
                         <div id="add_to_cart" {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE}style="display:none"{/if} class="buttons_bottom_block">
                            {if ($product->show_price && !isset($restricted_country_mode)) || isset($groups) || $product->reference || (isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS)}
                                ...
                                <form id="buy_block" style="margin-top:0" {if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
                                    <!-- hidden datas -->
                                    <button type="submit" name="Submit" class="exclusive lmromancaps">{l s='+ Agregar a la bolsa'}</button>
                                    <p class="hidden">
                    ...
                    {if $theme_settings.share_buttons == 1}						
                        ...
                        <!--<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-55d3a82c42c727c5" async="async"></script>-->
			...
			{if !$content_only}                
                {if isset($features) && $features|is_array && $features|@count gt 0}
                    {if $features|@count eq 1 && $features.0.id_feature eq 8}
                        {$show_features_tab = false}
                    {else}
                        {$show_features_tab = true}
                    {/if}
                {else}
                    {$show_features_tab = false}
                {/if}
				...
				<div class="tab-titles">
					<h3 class="active-tab" data-title="1">{l s='INFORMACIÓN'}</h3>{if $show_features_tab}<h3 data-title="2">{l s='CARACTERÍSTICAS'}</h3>{/if}{if isset($accessories) && $accessories}<h3 data-title="3">{l s='Accessories'}</h3>{/if}{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}<h3 data-title="4">{l s='Volume discounts'}</h3>{/if}{if isset($attachments) && $attachments}<h3 data-title="5">{l s='Download'}</h3>{/if}{if isset($product) && $product->customizable}<h3 data-title="6">{l s='Product customization'}</h3>{/if}{if isset($packItems) && $packItems|@count > 0}<h3 data-title="7">{l s='Pack content'}</h3>{/if}<h3 class='page-product-heading' data-title="12"><span>REVISA TU TALLA</span></h3>{$HOOK_PRODUCT_TAB}
				</div>			
				
				{if $product->description}
					<!-- More info -->
					<section class="page-product-box active-tab active-section" data-section="1">						
                ...
                {else}
                    <!-- More info -->
					<section class="page-product-box active-tab active-section" data-section="1">
                        <div  class="rte"><p>No hay descripción definida</p></div>						
					</section>
					<!--end  More info -->
				{/if}
                    ...
                    <section class="page-product-box" data-section="2">										
                        <ul class="table-data-sheet">			
                            {foreach from=$features item=feature}
                                {if isset($feature.value) && $feature.id_feature != 8}
...
$(window).load(function(){
	...
	$(".thickbox").fancybox();    
        ...
        $("#quantity_wanted").on("change", function() {
            var qty = $(this).val();
            if (!isNaN(qty)) {                
                var url_cart = "{/literal}{$link->getPageLink('cart', true, NULL, "id_product={$product->id|intval}&token={$static_token}")}{literal}";
                url_cart = url_cart + "&qty=" + qty + "&add";
                $(".box-cart-bottom a.btn-buy-single").attr("href", url_cart);
...
{addJsDefL name=doesntExistNoMore}{l s='Sin stock' js=1}{/addJsDefL}
