...
{capture name=path}{l s='Carro de compras'}{/capture}

<h1 id="cart_title" class="page-heading">{l s='CARRO DE COMPRAS'}
...
{else}
	...
	{if isset($lastProductAdded) AND $lastProductAdded}
		<div class="cart_last_product">
			...
			<a class="cart_last_product_img" href="{$link->getProductLink($lastProductAdded.id_product, $lastProductAdded.link_rewrite, $lastProductAdded.category, null, null, $lastProductAdded.id_shop)|escape:'html':'UTF-8'}">
				<img src="{$link->getImageLink($lastProductAdded.link_rewrite, $lastProductAdded.id_image, 'small_'|cat:$cookie->img_name)|escape:'html':'UTF-8'}" alt="{$lastProductAdded.name|escape:'html':'UTF-8'}"/>
			</a>
		...
		<table id="cart_summary" class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart_avail item hidden">{l s='Avail.'}</th>
					<th class="cart_unit item">{l s='PRECIO'}</th>
			...
			<tfoot>
				<!--<tr class="tr-cart-separator-02">
					<td colspan="7" class="cart-separator-02">&nbsp;</td>
				</tr>-->
				<tr class="cart_total_price">
					<td colspan="7" id="cart_voucher" class="cart_voucher">
						<!--<div class="shopping_cart_voucher">
							<div class="shopping_cart_voucher_content">
								<div class="space">
									{if $voucherAllowed}
										...
										<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
											<fieldset>
												<h4 class="hidden">{l s='Vouchers'}</h4>
									...
									<td class="text-right">
										{if $use_taxes}
											{if $display_tax_label}
												{if $priceDisplay}
													{l s='Total products (tax excl.)'}
												{else}
													{l s='Total products (tax incl.)'}		
												{/if}
											{else}
												{l s='Total products'}
											{/if}
									...
									{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
										{if $priceDisplay}
											<tr class="cart_total_delivery" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
												<td class="text-right">{if $display_tax_label}{l s='Total shipping (tax excl.)'}{else}{l s='Total shipping'}{/if}</td>
										...
										{else}
											<tr class="cart_total_delivery"{if $total_shipping <= 0} style="display:none;"{/if}>
												<td class="text-right">{if $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}</td>
												<td  class="price" id="total_shipping" >{displayPrice price=$total_shipping}</td>
											</tr>
										{/if}
									{else}
										<tr class="cart_total_delivery"{if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
											<td class="text-right">{l s='Total shipping'}</td>
											<td class="price" id="total_shipping" >{displayPrice price=$total_shipping_tax_exc}</td>
										</tr>
									{/if}
								{/if*}
	...
	<p class="cart_navigation clearfix">		
		<a
			...
			class="button-exclusive btn btn-default button standard-continue"
			title="{l s='Volver a la tienda'}">
			{l s='Volver a la tienda'}
		</a>
		{*if !$opc}
			<a
				href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}{/if}"
