		...
		<div class="paiement_block">
			<div id="HOOK_TOP_PAYMENT">{$HOOK_TOP_PAYMENT}</div>
				{if $HOOK_PAYMENT}
							...
							<table id="cart_summary" class="table table-bordered">
								...
								<tfoot>
									...
									{if $use_taxes}
										...
										{/if}
										<tr class="cart_total_price">
											<td colspan="3" class="total_price_container text-right">{l s='Total'}</td>
					...
					{if $opc}</div></div>
				</div> <!-- end opc_payment_methods-content -->{/if}
				{else}
			...
			{if !$opc}
					<p class="cart_navigation clearfix">
						<a href="{$link->getPageLink('order', true, NULL, "step=2")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default button">
